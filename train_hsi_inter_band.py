...
import setproctitle
setproctitle.setproctitle("gsplat-cholesky-HSI-inter")
...
class SimpleTrainer2d:
    ...
    def __init__(
        self,
        image_path: Path,
        num_points: int = 2000, # 800 1000 3000 5000 7000 9000
        ...
        self.prev_rgb = prev_rgb 
        self.prev_weight = prev_weight 
        
        self.num_points = num_points #800 1000 3000 5000 7000 9000
        image_path = Path(image_path)
        self.image_name = image_path.stem
        BLOCK_H, BLOCK_W = 16, 16
        self.H, self.W = self.gt_image.shape[2], self.gt_image.shape[3]
        self.iterations = iterations #50000
        self.save_imgs = args.save_imgs
        self.log_dir = Path(f"./checkpoints/{args.data_name}/{model_name}_{args.iterations}_{num_points}/{self.image_name}")
        # lr = 1e-3
        if model_name == "gaussianimage_cholesky_band":
            from gaussianimage_cholesky_band import GaussianImage_Cholesky
    ...
    def train(self):     
        ...
        for iter in range(1, self.iterations + 1):
            # 如果当前是第一轮 && 有上一次图像的特征（说明这不是第一张图）
            if iter <= 25000 and self.prev_xyz is not None:
                loss, psnr = self.gaussian_model.train_use_iter(
                    self.gt_image, iter,
                    xyz_coordinates=self.prev_xyz,
                    rgb_values=self.prev_rgb,
                    rgb_weight=self.prev_weight
                )
            else:
                # 正常训练
            ...
            if iter == self.iterations:
                prev_xyz = self.gaussian_model._xyz.detach().cpu().numpy()
                prev_rgb = self.gaussian_model._features_dc.detach().cpu().numpy()
                prev_weight = self.gaussian_model.rgb_W.detach().cpu().numpy()
            with torch.no_grad():
                if iter % 10 == 0:
        ...
        return psnr_value, ms_ssim_value, end_time, test_end_time, 1/test_end_time ,prev_xyz,prev_rgb,prev_weight
...
def parse_args(argv):
    ...
    parser.add_argument(
        "-d", "--dataset", type=str, default='./datasets/IP/', help="Training dataset"
    )
    parser.add_argument(
        "--data_name", type=str, default='IP', help="Training dataset"
    )
    parser.add_argument(
        "--iterations", type=int, default=50000, help="number of training epochs (default: %(default)s)"#50000
    )
    parser.add_argument(
        "--model_name", type=str, default="GaussianImage_Cholesky", help="model selection: GaussianImage_Cholesky_hsi, GaussianImage_RS, 3DGS"
    )
    parser.add_argument(
...
def main(argv):
    ...
    prev_xyz, prev_rgb, prev_weight = None, None, None
    for i in range(start, start+image_length):
        ...
        trainer = SimpleTrainer2d(image_path=image_path, num_points=args.num_points, 
            iterations=args.iterations, model_name=args.model_name, args=args, model_path=args.model_path,prev_xyz=prev_xyz,prev_rgb=prev_rgb,prev_weight=prev_weight)
        psnr, ms_ssim, training_time, eval_time, eval_fps, prev_xyz ,prev_rgb ,prev_weight = trainer.train()
        psnrs.append(psnr)
