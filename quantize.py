...
class FakeQuantizationCustom(torch.autograd.Function):
    """Performs fake quantization with customizable bit-width"""

    @staticmethod
    def forward(_, x, bitwidth=8):
        min_val = -(2**(bitwidth - 1))  # Example for signed numbers
        max_val = (2**(bitwidth - 1)) - 1
        scale = (max_val - min_val) / (x.max() - x.min())  # Scale to fit range
        quantized = torch.round(x * scale).clamp(min_val, max_val)  # Quantize
        dequantized = quantized / scale  # Dequantize
        return dequantized

    @staticmethod
    def backward(_, grad_output):
        # Gradient is unchanged (identity gradient)
        return grad_output
