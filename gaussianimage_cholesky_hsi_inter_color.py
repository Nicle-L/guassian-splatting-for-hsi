...
class GaussianImage_Cholesky(nn.Module):
    def __init__(self, loss_type="L2", **kwargs):
        ...
        if self.quantize:
            ...
            # codebook_dim=3：每个输入向量维度为3。codebook_size=8：每个量化器的代码本大小为 8（最多只能编码8种“原型向量”）。
            # num_quantizers=2：堆叠了两个残差量化器（Residual VQ），增强表达能力。vector_type="vector"：使用标准向量量化策略（而非 STE 或其他变体）。
            # kmeans_iters=5：用 K-means 聚类对代码本进行初始化，训练时迭代 5 次
            self.cholesky_quantizer = UniformQuantizer(signed=False, bits=6, learned=True, num_channels=3) # 均匀量化 cholesk矩阵压缩
            # signed=False 无符号量化，每通道6位量化，scale 和 beta 是可学习参数，num_channels=3用于三维输入
            
        #if self.quantize:
        #    self.xyz_quantizer = FakeQuantizationHalf.apply 
        #    #self.xyz_quantizer=FakeQuantizationCustom.apply 
        #    self.features_dc_quantizer = VectorQuantizer(codebook_dim=3, codebook_size=2, num_quantizers=1, vector_type="vector", kmeans_iters=1) #codebook_size、kmeans_iters
        #    self.cholesky_quantizer = UniformQuantizer(signed=False, bits=2, learned=True, num_channels=3)    #bits
